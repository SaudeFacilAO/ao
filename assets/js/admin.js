...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const dias = document.querySelectorAll(".checkbox-group input");
...
//««««««««««««««««««««Listar Resultado do Relatório«««««««««««««««««««««««««
async function carregarResultadoRelatorio() {
    try {
        const res = await fetch(API + "list_relatorio.php");
        const result = await res.json();

        const tbody = document.querySelector(".data-table #reportTableBody");
        tbody.innerHTML = "";

        if (result.status !== "ok") {
            alert(result.msg || "Erro ao carregar relatórios");
            return;
        }

        result.data.forEach(r => {

            const ativoTexto = r.tipo == "teleconsulta" ? "Presencial" : "Online";
            const ativoClass = r.tipo == "teleconsulta" ? "presencial" : "online";

            const completoTexto = r.status == "realizada" ? "Concluído" : "Online";
            const completoClass = r.status == "realizada" ? "completed" : "online";
            ...
            tbody.innerHTML += `
                <tr>
                    ...
                    <td>
                        <span class="badge-pill ${ativoClass}">
                             ${ativoTexto}
                        </span>
                    </td>
                    <td>
                        <span class="badge-status ${completoClass}">
                        <i class="fas fa-check-circle"></i> 
                            ${completoTexto}
                        </span>
                    </td>
                    <td>
                        <button class="btn-action view-report" title="Visualizar relatório" onclick="verRelatorio(${r.id})">
                ...
                <tr>
                    ...
                    <td>
                        <button class="btn-action delete"
                            title="Remover" onclick="eliminarEspecialidade(${e.id})">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const secretarioGroup = document.getElementById("secretarioGroup");
    const teleconsultaGroup = document.getElementById("teleconsultaGroup"); // 👈 NOVO
    ...
    function hideAll() {
        ...
        if (especialidadeGroup) {
            ...
            especialidadeGroup.querySelectorAll("input, select").forEach(i => i.value = "");
        }
        ...
        if (secretarioGroup) {
            secretarioGroup.style.display = "none";
        }

        if (teleconsultaGroup) { // 👈 NOVO
            teleconsultaGroup.style.display = "none";
            const select = teleconsultaGroup.querySelector("select");
    ...
    function handleRoleChange() {
        ...
        if (role.value === "medico") {
            ...
            if (especialidadeGroup) especialidadeGroup.style.display = "block";
            if (hospitalGroup) hospitalGroup.style.display = "block";
            if (teleconsultaGroup) teleconsultaGroup.style.display = "block"; // 👈 NOVO
        }

        if (role.value === "secretario") {

            if (secretarioGroup) {
                secretarioGroup.style.display = "block";
            }
        }
    }
...
async function carregarUsuarios() {
    try {
        ...
        const tbody = document.querySelector(".users-table #usuariosTableBody");
        tbody.innerHTML = "";
        ...
        result.data.forEach(u => {
            ...
            tbody.innerHTML += `
                <tr>
                    ...
                    <td>
                        <button class="action-btn edit" title="Editar" onclick="editUser(${u.id})"><i class="fas fa-edit"></i></button>
                        <button class="action-btn delete" title="Deletar" onclick="deleteUser(${u.id})"><i class="fas fa-trash"></i></button>
                    </td>
                </tr>
            `;
        });
...
function obterDiasSelecionados() {
    const dias = document.querySelectorAll(".checkbox-group input:checked");
                ...
                <div class="escala-card">
                    ...
                    <div class="escala-actions">
                        ...
                        <button
                            ...
                            onclick="editarEscala(${medico.medico_id})">
                        ...
                        <button
                            ...
                            onclick="eliminarEscala(${medico.medico_id})">
...
// ===== SPECIALTY FUNCTIONS =====
function openEspecialidadeForm() {
    const modal = document.getElementById('especialidadeModal');
    if (modal) {
        modal.style.display = 'flex';
    }
}

function closeEspecialidadeForm() {
    const modal = document.getElementById('especialidadeModal');
    if (modal) {
...
function openScaleForm() {
    const modal = document.getElementById('scaleFormModal');
    if (modal) {
        modal.style.display = 'flex';
    }
}
...
function setupButtonListeners() {
    const btnCadastrarEsp = document.getElementById('btnCadastrarEsp');
    const btnNovaEscala = document.getElementById('btnNovaEscala');

    if (btnCadastrarEsp) {
        btnCadastrarEsp.addEventListener('click', openEspecialidadeForm);
    }

    if (btnNovaEscala) {
        btnNovaEscala.addEventListener('click', openScaleForm);
    }

    // Form submission handlers
    const formEspecialidade = document.getElementById('formEspecialidade');
    if (formEspecialidade) {
        formEspecialidade.addEventListener('submit', function (e) {
            e.preventDefault();
            const nome = document.getElementById('nomeEspecialidade').value;
            const descricao = document.getElementById('descricaoEspecialidade').value;
            const status = document.getElementById('statusEspecialidade').value;
            ...
            if (nome && descricao) {
                ...
                tr.innerHTML = `
                    ...
                    <td>${descricao}</td>
                    <td><span class="status-indicator ${status}"></span> ${status.charAt(0).toUpperCase() + status.slice(1)}</td>
                    <td>
